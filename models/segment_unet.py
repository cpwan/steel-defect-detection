...
def accuracy(output,target):
    return metric_dice.dice(output,target)
...
def train_seg(args):
    ...
    single_model = unet.Unet(classes=args.classes)
    if args.pretrained:
        single_model.load_state_dict(torch.load(args.pretrained))
    model = torch.nn.DataParallel(single_model).cuda()
#    criterion = nn.NLLLoss(ignore_index=255)
    criterion = metric_dice.DICELoss(ignore_index=255)
    criterion.cuda()
...
# Produce a 2d histogram where row is label and column is prediction
...
def cal_iou(pred, label, num_classes):
    """
    pred (predicted class label): N, H, W
    label (GT class label): N, H, W

    Return (IoU per prediction and class): BATCH_SIZE, NUM_CLASSES
    """
    batch_size = pred.shape[0]
    result = np.zeros([batch_size, num_classes])
    for i in range(0, batch_size):
        hist = fast_hist(pred[i].flatten(), label[i].flatten(), num_classes)
        result[i] = per_class_iu(hist)
    return result
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        # Softmax
        final = nn.functional.softmax(final, 1)

        final = final.cuda()
        label = label.cuda()
        dice_list.append(accuracy(final, label))
        final = final.cpu()
        label = label.cpu()

        # Get the indices of max likelihood
        _, pred = torch.max(final, 1)
        pred1 = process_output(final)
        pred = pred.cpu().data.numpy()
        pred1 = pred1.cpu().data.numpy()
        batch_time.update(time.time() - end)
        if save_vis:
            save_output_images(pred1, name, output_dir)
            save_colorful_images(
                pred1, name, output_dir + '_color',
        ...
        if has_gt:
            ...
            logger.info('===> mAP {mAP:.3f}'.format(
                mAP=round(np.nanmean(dice_list) * 100, 2)))
        end = time.time()
